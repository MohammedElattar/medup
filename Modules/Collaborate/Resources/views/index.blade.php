...
@section('title', translate_ui('collaborates'))
@section('content')
    <x-ui.table.view :columns="['ID', 'Title', 'college', 'speciality', 'price', 'expert', 'comments_count', 'date', 'Actions']"
                     :title="'collaborates'"
                     :paginationObject="PaginationHelper::getPaginationObject($collaborates)">
        <x-ui.table.body>
            @foreach($collaborates->items() as $item)
                <tr>
                    <td>{{ $item->id }}</td>
                    <td>{{ $item->title }}</td>
                    ...
                    <td>{{ $item->expert->user->first_name ?? 'N/A' }}</td>
                    ...
                    <x-ui.table.actions.view>
                        ...
                        <a class="dropdown-item w-auto" href="{{route('collaborates.change_status', $item->id)}}"
                           title="{{translate_ui($item->status ? 'deactivate' : 'activate')}}">
                            <i class="fa fa-{{$item->status ? 'cancel' : 'check'}}"></i>
                        </a>
                        <a class="dropdown-item w-auto" href="{{route('collaborates.edit', $item->id)}}"
                           title="{{translate_ui('show')}}">
                            <i class="fa fa-eye"></i>
                        </a>
                        <x-ui.table.actions.delete url="{{ route('collaborates.destroy', $item->id) }}"/>
                    </x-ui.table.actions.view>
                </tr>
            @endforeach
        </x-ui.table.body>

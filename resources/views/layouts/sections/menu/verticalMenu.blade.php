  ...
  @if(!isset($navbarFull))
    <div class="app-brand demo">
      <a href="{{url('/')}}" class="app-brand-link">
        <span>@include('_partials.macros',["height"=>20])</span>
    ...
    @foreach ($menuData[0]->menu as $menu)
      ...
      @if (isset($menu->menuHeader))
        <li class="menu-header small">
          <span class="menu-header-text">{{ translate_word($menu->menuHeader) }}</span>
        </li>
      @else
          ...
          <li class="menu-item {{$activeClass}}">
            <a href="{{ isset($menu->url) ? url($menu->url) : 'javascript:void(0);' }}"
              ...
              @isset($menu->icon)
                <i class="{{ $menu->icon }}"></i>
              @endisset
              <div>{{ isset($menu->name) ? translate_ui($menu->name) : '' }}</div>
              @isset($menu->badge)

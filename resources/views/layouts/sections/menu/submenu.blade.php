  ...
  @if (isset($menu))
    ...
    @php
      ...
      elseif (isset($submenu->submenu)) {
        if (gettype($submenu->submenu) === 'array') {
          foreach($submenu->submenu as $slug){
            if (str_contains($currentRouteName,$slug->slug) and strpos($currentRouteName,$slug->slug) === 0) {
                $activeClass = $active;
            }
          }
        }
    ...
    @endphp
      ...
      <li class="menu-item {{$activeClass}}">
        <a href="{{ isset($submenu->url) ? url($submenu->url) : 'javascript:void(0)' }}" class="{{ isset($submenu->submenu) ? 'menu-link menu-toggle' : 'menu-link' }}" @if (isset($submenu->target) and !empty($submenu->target)) target="_blank" @endif>
          ...
          @endif
          <div>{{ isset($submenu->name) ? translate_ui($submenu->name) : '' }}</div>
          @isset($submenu->badge)
